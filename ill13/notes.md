...
Quest Generation is Slightly Broken: In MapManager.generateNewMap(), you call QuestLogic.generateQuest(this.gameState, 0, this.seed, 1);. You're passing 0 as the currentLocationIndex. This means the quest will always be generated from the first location in the array, not necessarily the central hub or the player's starting point. It’s minor, but for consistency, you should pass the index of the centralLocation you just picked.

Performance on Mobile: The MapRenderer does a lot of DOM manipulation for the location markers. For a 20x15 grid with 8 locations, it’s fine now, but if you add more, consider using a single <canvas> for everything (terrain + markers) for buttery smooth performance.
